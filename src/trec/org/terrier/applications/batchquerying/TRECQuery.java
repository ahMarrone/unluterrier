...
import org.terrier.querying.strategies.NullIntersectionStrategy;
/**
...
public class TRECQuery implements QuerySource {
	...
	/** Use intersectionStrategy? Set using <tt>trec.querying.intersectionStrategy</tt>. Defaults to NullIntersectionStrategy */
	protected IntersectionStrategy intersectionStrategy;
		...
		try {
			...
			if (! Files.exists(queryfilename) || ! Files.canRead(queryfilename)) {
				...
				while (!queryTokenizer.isEndOfFile()) {
					...
					if (query.length() == 0)
						continue;
					vecStringQueries.add(this.intersectionStrategy.expandQuery(query.toString().trim()));
	...
	public TRECQuery() {
		...
		try {
			...
			this.intersectionStrategy = this.getIntersectionStrategy();
	...
	/** Obtain the intersection strategy. Loads the class specified by property <tt>trec.querying.strategies.intersectionStrategy</tt> */
	protected IntersectionStrategy getIntersectionStrategy() {
		IntersectionStrategy strategy = null;
		try {
			String className = ApplicationSetup.getProperty(
					"trec.querying.strategies.intersectionStrategy", NullIntersectionStrategy.class
							.getName());
			...
			strategy = Class.forName(className).asSubclass(IntersectionStrategy.class).newInstance();
		} catch (Exception e) {
			logger.error("", e);
		}
		return strategy;
